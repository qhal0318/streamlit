...
try:
    ...
    title_icon = Image.open("1-794df7f8.ico")
except FileNotFoundError:
    st.error("제목 아이콘 파일(1-794df7f8.ico)을 찾을 수 없습니다. app.py와 같은 폴더에 있는지 확인해주세요.")
    # 파일을 못 찾으면 대신 네잎클로버 이모티콘을 사용
    title_icon = "🍀" 

# --- 2. 페이지 기본 설정 ---
st.set_page_config(
    page_title="광고 어뷰징 탐지 센서",
    page_icon= title_icon, # 브라우저 탭 아이콘을 감자 이모티콘으로 설정
    layout="wide"
)
...
with col1:
    st.image(title_icon, width=70) # 페이지 제목 옆 아이콘 크기 70으로 키우기
