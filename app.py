...
# --- 2. 페이지 기본 설정 (여기서 브라우저 탭 아이콘도 설정) ---
st.set_page_config(
    page_title="광고 어뷰징 탐지 센서",
    page_icon=icon, # 브라우저 탭 아이콘 설정
    layout="wide"
)
col1, col2 = st.columns([1, 10]) # [아이콘 차지할 비율, 제목 차지할 비율]

with col1:
    st.image(icon, width=60) # 아이콘 크기는 width로 조절

with col2:
    st.title("광고 어뷰징 탐지 센서")
...
st.markdown(
    ...
    <div class="title-align">
        <img src="data:image/png;base64,{
            ...
            print(img_str) # 이 부분을 삭제하면 코드가 제대로 작동합니다
        }" width="70">  <div style="width: 15px;"></div> {st.title("광고 어뷰징 탐지 센서", anchor=False).to_html()}
    </div>
    """,
    unsafe_allow_html=True
)
